...
export class PfxReader extends Mixin(LocalFileOpenTrait) {
    public static createCredentialFromContents(
        contents: string,
        passPhrase: string
    ): Credential {
        if (contents === '') {
        ...
        return Credential.create(
            pki.certificateToPem(pfx.cert),
            pki.privateKeyToPem(pfx.pKey),
            ''
        );
    }

    public static createCredentialFromFile(
        filename: string,
        passPhrase: string
    ): Credential {
        return this.createCredentialFromContents(
            this.localFileOpen(filename),
            passPhrase
        );
    }
    ...
    public static loadPkcs12(
        contents: string,
        password = ''
    ): { cert: pki.Certificate; pKey: pki.PrivateKey } {
        try {
